...
func serialize_run_state()->Dictionary:
	...
	for key in current_run_state.effects:
		if key == "starting_item":
			...
			for item in current_run_state.effects[key]:
				serialized_effects[key].push_back(item.my_id)
			
		elif RunData.effect_keys_full_serialization.has(key):
...
func deserialize_run_state(state:Dictionary)->Dictionary:
	...
	for active_set_effect in state.active_set_effects:
		for effect in ItemService.effects:
			if effect.get_id() == active_set_effect[1].effect_id:
				...
				deserialized_run_state.push_back([active_set_effect[0], deserialized_effect])
				break
	...
	for key in state.effects:
		if key == "starting_item":
			...
			for item_id in state.effects[key]:
				var item_data = ItemService.get_element(ItemService.items, item_id)
				var weapon_data = ItemService.get_element(ItemService.weapons, item_id)
				
				if item_data != null:
					deserialized_effects[key].push_back(item_data)
				elif weapon_data != null:
					deserialized_effects[key].push_back(weapon_data)
