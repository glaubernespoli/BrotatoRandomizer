...
func init_melee_stats(from_stats:MeleeWeaponStats = MeleeWeaponStats.new(), weapon_id:String = "", weapon_classes:Array = [], effects:Array = [], is_structure:bool = false)->MeleeWeaponStats:
	var new_stats = init_base_stats(from_stats, weapon_id, weapon_classes, effects, is_structure) as MeleeWeaponStats
...
func init_ranged_stats(from_stats:RangedWeaponStats = RangedWeaponStats.new(), weapon_id:String = "", weapon_classes:Array = [], effects:Array = [], is_structure:bool = false)->RangedWeaponStats:
	
	var new_stats = init_base_stats(from_stats, weapon_id, weapon_classes, effects, is_structure) as RangedWeaponStats
	...
	if from_stats.increase_projectile_speed_with_range:
		new_stats.projectile_speed = min(6000, from_stats.projectile_speed + (from_stats.projectile_speed / 300.0) * Utils.get_stat("stat_range")) as int
	else :
...
func init_base_stats(from_stats:WeaponStats, weapon_id:String = "", weapon_classes:Array = [], effects:Array = [], is_structure:bool = false)->WeaponStats:
	...
	for class_bonus in RunData.effects["weapon_class_bonus"]:
		if weapon_classes.has(class_bonus[0] as int):
	...
	new_stats.damage = max(1.0, new_stats.damage + get_scaling_stats_value(base_stats.scaling_stats))
	...
	new_stats.knockback = max(0.0, base_stats.knockback + RunData.effects["knockback"])
