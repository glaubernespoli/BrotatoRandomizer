...
func check_stat_challenges()->void :
	var chal_hallucination = get_chal("chal_hallucination")
	var chal_fast = get_chal("chal_fast")
	var chal_slow = get_chal("chal_slow")
	var chal_dying = get_chal("chal_dying")
	var chal_perfect_vision = get_chal("chal_perfect_vision")
	var chal_agriculture = get_chal("chal_agriculture")
	
	if Utils.get_stat("stat_dodge") >= chal_hallucination.value:
		complete_challenge("chal_hallucination")
	
	if Utils.get_stat("stat_speed") >= chal_fast.value:
		complete_challenge("chal_fast")
	
	if Utils.get_stat("stat_speed") <= chal_slow.value:
		complete_challenge("chal_slow")
	
	if Utils.get_stat("stat_range") >= chal_perfect_vision.value:
		complete_challenge("chal_perfect_vision")
	
	if Utils.get_stat("stat_hp_regeneration") <= chal_dying.value:
		complete_challenge("chal_dying")
	
	if Utils.get_stat("stat_harvesting") >= chal_agriculture.value:
		complete_challenge("chal_agriculture")
