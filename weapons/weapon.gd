...
func _on_Hitbox_hit_something(thing_hit:Node, damage_dealt:int)->void :
	...
	for effect in effects:
		if effect is ExplodingEffect and randf() < effect.chance:
			WeaponService.explode(effect, thing_hit.global_position, _hitbox.damage, _hitbox.accuracy, _hitbox.crit_chance, _hitbox.crit_damage, _hitbox.burning_data, _hitbox.is_healing, [thing_hit])
	
	on_weapon_hit_something(thing_hit, damage_dealt)


func on_weapon_hit_something(thing_hit:Node, damage_dealt:int)->void :
