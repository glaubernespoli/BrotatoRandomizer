...
func _ready()->void :
	...
	_wave_timer.wait_time = 1 if DebugService.instant_waves else current_wave_data.wave_duration
	_wave_timer.start()
	...
	if RunData.effects["gain_pct_gold_start_wave"] != 0:
		var val = RunData.gold * (RunData.effects["gain_pct_gold_start_wave"] / 100.0)
...
func _on_enemy_died(enemy:Enemy)->void :
	...
	if not _cleaning_up:
		...
		if randf() < RunData.effects["dmg_when_death_from_luck"][0] / 100.0 and _entity_spawner.get_all_enemies().size() > 0:
			...
			var dmg_taken = other_enemy.take_damage(dmg)
		...
		if RunData.effects["explode_on_death"].size() > 0:
			...
			if randf() <= explosion_chance:
				...
				WeaponService.explode(first, enemy.global_position, dmg, first.stats.accuracy, first.stats.crit_chance, first.stats.crit_damage, first.stats.burning_data, false, [], "item_rip_and_tear")
...
func on_gold_picked_up(gold:Node)->void :
	...
	if randf() < RunData.effects["heal_when_pickup_gold"] / 100.0:
		if _player.current_stats.health < _player.max_stats.health:
			RunData.tracked_item_effects["item_cute_monkey"] += 1
	...
	if gold.attracted_by == _player:
		if randf() < RunData.effects["dmg_when_pickup_gold_from_luck"][0] / 100.0 and _entity_spawner.get_all_enemies().size() > 0:
			...
			var dmg_taken = enemy.take_damage(dmg)
	...
	if RunData.effects["stats_end_of_wave"].size() > 0:
		for stat_end_of_wave in RunData.effects["stats_end_of_wave"]:
			...
			if stat_end_of_wave[0] == "stat_percent_damage":
				...
				var nb_of_vigilante_rings = 0
				var vigilante_ring_value = 0
				
				for item in RunData.items:
					if item.my_id == "item_vigilante_ring":
						nb_of_vigilante_rings += 1
						vigilante_ring_value = item.effects[0].value
				
				RunData.tracked_item_effects["item_vigilante_ring"] += (nb_of_vigilante_rings * vigilante_ring_value)
