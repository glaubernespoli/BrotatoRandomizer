...
func set_data(weapon_class:int)->void :
	var set = ItemService.get_set(weapon_class)
	var nb = min(set.set_bonuses.size() + 1, RunData.active_sets[weapon_class]) as int if RunData.active_sets.has(weapon_class) else 0
	
	_synergy_name.text = tr(ItemService.get_weapon_class_text(weapon_class))
