...
func _ready()->void :
	...
	if RunData.max_endless_wave_record_beaten != - 1:
		...
		var arg0 = "[color=" + Utils.POS_COLOR_STR + "]" + str(RunData.max_endless_wave_record_beaten) + "[/color]"
		var arg1 = "[color=" + Utils.POS_COLOR_STR + "]" + str(Text.text(difficulty_data.name, [str(RunData.get_current_difficulty())])) + "[/color]"
		var arg2 = "[color=" + Utils.POS_COLOR_STR + "]" + str(Utils.get_enemy_scaling_text(
		...
		difficulty_data.description = Text.text("NEW_ENDLESS_WAVE_HIGH_SCORE", [arg0, arg1, arg2])
		
		RunData.challenges_completed_this_run.push_back(difficulty_data)
