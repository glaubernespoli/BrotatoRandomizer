...
func set_element(character_id:String)->void :
	...
	if character_diff_data.max_endless_wave_beaten.wave_number >= 0:
		...
		_max_endless_value.text = "%s - %s%s" % [Text.text("WAVE", [str(character_diff_data.max_endless_wave_beaten.wave_number)]), Text.text(max_endless_diff_data.name, [str(max_endless_diff_data.my_id)]), scaling_text]
	else :
